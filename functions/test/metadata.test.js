...
describe('Metadata Fetcher Tests', () => {
  ...
  test('YouTube thumbnail extraction', () => {
    ...
    const images = fetchFromYouTube(url);
    assert.ok(images.length > 0);
    assert.ok(images[0].includes('dQw4w9WgXcQ'));
  });
  ...
  test('Image extraction from meta tags', () => {
    ...
    assert.ok(images.includes('https://example.com/img.jpg'));
  });
  ...
  test('Subject samples verification', async () => {
    ...
    for (const s of subjects) {
      if (s.url && s.url.includes('youtube.com')) {
        const images = fetchFromYouTube(s.url);
