...
void main() {
  ...
  test('Key Rotation (Replace)', () async {
    ...
    final TrustStatement sCharlie = await bob.trust(charlie, moniker: 'charlie');  // Charlie was trusted by the OLD key
    ...
    final TrustPipeline pipeline = TrustPipeline(source, maxDegrees: 6, pathRequirement: (int d) => 1);
    final TrustGraph graph = await pipeline.build(alice.id);
  ...
  test('Confidence Levels (Multiple Paths)', () async {
    ...
    final TrustPipeline pipeline = TrustPipeline(source, pathRequirement: (int d) => d >= 2 ? 2 : 1);
  ...
  test('Node-Disjoint Paths (Bottleneck Test)', () async {
    ...
    final TrustPipeline pipeline = TrustPipeline(source, pathRequirement: (int d) => d >= 3 ? 2 : 1);
  ...
  test('Replace Notification (Not Conflict)', () async {
    ...
    expect(graph.notifications.any((TrustNotification n) => !n.isConflict && n.reason.contains('being replaced')), isTrue);
  });
  ...
  test('Trust Non-Canonical Key Notification', () async {
    ...
    expect(graph.isTrusted(bobNew.id), isTrue);
    expect(graph.notifications.any((TrustNotification n) => n.reason.contains('non-canonical')), isFalse, reason: "Should not notify for others' mistakes");
    ...
    expect(graph2.notifications.any((TrustNotification n) => n.reason.contains('is being replaced by')), isTrue, 
  ...
  test('Distance Authority: Deep Replacement Constraint Ignored', () async {
    ...
    expect(graphAlice.notifications.any((TrustNotification n) => n.reason.contains('Replacement constraint ignored due to distance')), isTrue);
  ...
  test('Graph Paths (Shortest Paths)', () async {
    ...
    expect(paths, containsAll(<List<IdentityKey>>[
  ...
  test('Replacement: Far to Close (Identity Link vs Replacement Constraint)', () async {
    ...
    expect(graph.notifications.any((TrustNotification n) => n.reason.contains('Replacement constraint ignored due to distance')), isTrue);
  });
}
