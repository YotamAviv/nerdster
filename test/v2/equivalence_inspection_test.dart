...
void main() {
  ...
  test('Equivalence Inspection: Can inspect merged subject', () async {
    ...
    final FollowNetwork followNetwork = reduceFollowNetwork(graph, delegateResolver, ContentResult(), kFollowContextNerdster);
    ...
    final V2Labeler labeler = V2Labeler(graph, delegateResolver: delegateResolver, meIdentity: lisa.id);
    ...
    // Toy token:
    final Map<String, dynamic> toy = createTestSubject(type: ContentType.resource, title: 'Toy', url: 'https://en.wikipedia.org/wiki/Toy');
    final String toyToken = getToken(toy);
    
    // Skateboard token:
    final Map<String, dynamic> skateboard = createTestSubject(type: ContentType.resource, title: 'Skateboard', url: 'https://en.wikipedia.org/wiki/Skateboard');
