...
channelFactory.register(
  domain: 'foreign-service.com',
  exportUrl: 'https://export.foreign-service.com',
);
...
Example: Carl and Smithers are foreign (their trust statements live on `foreign-domain.com`).
...
**`simpsons_demo.dart`** — two things for Carl and Smithers:
...
| `simpsons_demo.dart` | Register Carl/Smithers as `FedKey`s; create with `trustDomain` | Small |
