# ContentView V2 Specification

## Overview
The `ContentView` V2 is the successor to the legacy `ContentTree`. It moves away from a strict hierarchical tree structure in favor of a more fluid, subject-centric aggregation that is optimized for both discovery and social proof. It follows a testable MVC architecture:
- **Model**: `V2FeedModel` holds the immutable snapshot of the aggregated feed.
- **View**: `ContentView` and `ContentCard` handle the UI rendering.
...
- **MVC Pattern**: Decouples data processing (`V2FeedController`), state representation (`V2FeedModel`), and UI rendering (`ContentView`).
...
| Feature | ContentTree (Legacy) | ContentView (V2) |
|---------|----------------------|-----------------------|
