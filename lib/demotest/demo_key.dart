...
const demoData = ${encoder.convert(_exports)};''';
  ...
  DemoIdentity get asIdentity => DemoIdentity(this);
  DemoDelegate get asDelegate => DemoDelegate(this);
}

class DemoIdentity {
  final DemoKey key;
  DemoIdentity(this.key);
  String get name => key.name;
  IdentityKey get id => IdentityKey(key.token);

  Future<TrustStatement> trust(DemoIdentity other,
      {required String moniker, String? comment, String? export}) async {
    return key.trust(other.key, moniker: moniker, comment: comment, export: export);
  }

  Future<TrustStatement> block(DemoIdentity other, {String? comment, String? export}) async {
    return key.block(other.key, comment: comment, export: export);
  }

  Future<TrustStatement> replace(DemoIdentity other,
      {Statement? lastGoodToken, String? comment, String? export}) async {
    return key.replace(other.key,
        lastGoodToken: lastGoodToken, comment: comment, export: export);
  }

  Future<TrustStatement> delegate(DemoKey other, // Accepts DemoKey as it could be fresh
      {required String domain, String? comment, String? export}) async {
    return key.delegate(other, domain: domain, comment: comment, export: export);
  }

  Future<TrustStatement> clear(DemoIdentity other) async {
    return key.clear(other.key);
  }

  Future<DemoKey> makeDelegate({String? export}) => key.makeDelegate(export: export);
}

class DemoDelegate {
  final DemoKey key;
  DemoDelegate(this.key);
  String get name => key.name;
  DelegateKey get id => DelegateKey(key.token);
  ...
  Future<ContentStatement> doFollow(dynamic subject, Json contexts,
      {ContentVerb verb = ContentVerb.follow, String? export}) async {
    return key.doFollow(subject, contexts, verb: verb, export: export);
  }
