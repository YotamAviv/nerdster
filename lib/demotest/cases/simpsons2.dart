...
Future<(DemoKey, DemoKey?)> simpsons2() async {
  ...
  DemoKey hugh = await DemoKey.findOrCreate('hugh');
  DemoKey dick = await DemoKey.findOrCreate('dick');
  DemoKey ben = await DemoKey.findOrCreate('ben');
  await sideshow.doTrust(TrustVerb.trust, mel, moniker: 'Mel');
  await sideshow.doTrust(TrustVerb.trust, milhouse, moniker: '4-Eyes');
  await sideshow.doTrust(TrustVerb.trust, hugh,
  ...
  await sideshow.doTrust(TrustVerb.trust, dick, moniker: 'Dick Hurtz'); // Who's Dick Hurtz?
  await sideshow.doTrust(TrustVerb.trust, ben,
  ...
  // milhouse->sideshow, marge-x> sideshow
  await marge.doTrust(TrustVerb.block, sideshow);
  await milhouse.doTrust(TrustVerb.trust, sideshow, moniker: 'Sideshow T. C.');
  ...
  DemoKey hughN = await hugh.makeDelegate();
  ...
  await hughN.doRate(subject: shakes, recommend: true, verb: ContentVerb.rate);
